...
// --- Notification logic (copied from messages.js) ---
...
function initializeNotificationSocket(username) {
    if (!window._notificationSocketInitialized && window.socket) {
        window._notificationSocketInitialized = true;
        window.socket.emit("registerUser", username);
        window.socket.on("newMessage", ({ chatId, message, chatName }) => {
            if (
